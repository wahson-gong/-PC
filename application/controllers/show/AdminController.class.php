         ...
         //修改密码
          ...
          //昵称
            ...
            public function shAction(){
                ...
                $sql = ' select pl.*,m.mingcheng as uname from sl_pinglun as pl left join sl_member as m on pl.uid = m.id where pl.xiangzhen="'.$xiangzhen.'" ';
                $nickname = '';
                if(isset($_GET['nickname']) && !empty($_GET['nickname'])){
                  $nickname = $_GET['nickname'];
                  $sql .= ' and locate("'.$nickname.'",m.mingcheng)';
                }
                $sql .= " limit {$act},{$_num} ";

                $msg=$_model->findBySql($sql);
                //>>获取评论人名称
                foreach ($msg as $k=>$v){
                    $uid = $v['uid'];
